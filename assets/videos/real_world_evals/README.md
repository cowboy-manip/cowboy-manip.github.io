...
The grid shows lightweight poster frames by default. Hovering or focusing a card starts playback, and that video keeps playing after the pointer leaves. Scenes with multiple videos show a `Change object` button on hover/focus; clicking it advances only that scene to its next video. The page keeps at most six evaluation videos playing at once; selecting another video unloads the oldest active one. Videos also stop and unload when their card leaves the viewport.
